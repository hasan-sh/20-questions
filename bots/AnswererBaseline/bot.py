...
class AnswererBase:
    """
    The Class representing the Outlier Baseline Answerer in case there is no human player.
    ...
    pickEntity()
        Queries the KG to randomly select an entity that has both a type and a label, with the least amount of predicates. 
    ...
    def pickEntity(self):
        ...
        """
        query ="""
        SELECT ?s (COUNT(?p) as ?predicates)
            WHERE {
                ?s a ?_.
