...
class Game:
    ...
    """
    def __init__(self, state=None, nQuestions=constants.QUESTIONS_LIMIT, questioner=None, againstHuman = True, devMode = False, params = None): # TODO: load players dynamically.
        self.nQuestions = nQuestions
        self.state = state
        # Players: [Questioner, Answerer]
        if questioner:
            bot = helpers.load_bot(questioner)
            self.questioner = bot(self.state, params = params)
        else:
